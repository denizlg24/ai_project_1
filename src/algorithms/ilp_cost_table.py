...
class ILPCostTableSolver(Algorithm):
    ...
    def parameters(cls) -> list[ParameterDef]:
        return [
            ...
            ParameterDef("acc_weight", "Peso Contabilístico", float, 0.5, 0.0, 5.0, 0.1),
        ]
    ...
    def run(self, progress_callback: Callable[[ProgressData], None] | None = None) -> float:
        ...
        prob = pulp.LpProblem("ILP_CostTable", pulp.LpMinimize)
        ...
        for fid in range(self.problem.num_families):
            ...
            for i in range(self.max_choices):
                day = choices[i]
                pref_cost = Problem._preference_cost(size, choices, day)
                est_acc_cost = (occ_counts[day] / MAX_OCCUPANCY) * 50 
                obj_terms.append(x[fid, day] * (pref_cost + self.acc_weight * est_acc_cost))
        ...
        for day in range(1, N_DAYS + 1):
            relevant_vars = [x[fid, day] * self.problem.family_size[fid] for fid in range(self.problem.num_families) if (fid, day) in x]
            if relevant_vars:
                prob += pulp.lpSum(relevant_vars) <= MAX_OCCUPANCY
                prob += pulp.lpSum(relevant_vars) >= MIN_OCCUPANCY

...
class ILPHybridSolver(Algorithm):
    def __init__(self, problem: Problem):
        super().__init__(problem)
        self.max_choices = 6
        self.time_limit = 60
        self.acc_weight = 0.5
        self.smooth_weight = 5.0

    @classmethod
    def name(cls) -> str:
        return "ILP Híbrido (Tabela + Smooth)"

    @classmethod
    def parameters(cls) -> list[ParameterDef]:
        return [
            ParameterDef("max_choices", "Opções", int, 6, 1, 10, 1),
            ParameterDef("time_limit", "Tempo (s)", int, 60, 10, 3600, 10),
            ParameterDef("acc_weight", "Peso Tabela", float, 0.5, 0.0, 5.0, 0.1),
            ParameterDef("smooth_weight", "Peso Smoothness", float, 5.0, 0.0, 50.0, 0.5),
        ]
    ...
    def configure(self, **params: Any) -> None:
        ...
        self.acc_weight = float(params.get("acc_weight", self.acc_weight))
    ...
    def run(self, progress_callback: Callable[[ProgressData], None] | None = None) -> float:
        ...
        prob = pulp.LpProblem("ILP_Hybrid", pulp.LpMinimize)
        occ_counts = self.problem.daily_occupancy
        ...
        for fid in range(self.problem.num_families):
            ...
            for day in choices[:self.max_choices]:
                p_cost = Problem._preference_cost(size, choices, day)
                t_cost = (occ_counts[day] / MAX_OCCUPANCY) * 50
                pref_and_table.append(x[fid, day] * (p_cost + self.acc_weight * t_cost))
        
        prob += pulp.lpSum(pref_and_table) + (self.smooth_weight * 0.2) * pulp.lpSum([diff[d] for d in range(1, N_DAYS)])

        for fid in range(self.problem.num_families):
            prob += pulp.lpSum([x[fid, day] for day in self.problem.choices[fid][:self.max_choices]]) == 1
        for day in range(1, N_DAYS + 1):
            prob += pulp.lpSum([x[fid, day] * self.problem.family_size[fid] for fid in range(self.problem.num_families) if (fid, day) in x]) == occ[day]
